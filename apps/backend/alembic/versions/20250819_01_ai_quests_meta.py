...
def upgrade():
    ...
    if _has_table(insp, "quests"):
        ...
        if not _has_column(insp, "quests", "cost_generation"):
            op.add_column(
                "quests", sa.Column("cost_generation", sa.Integer(), nullable=True)
            )
    ...
    if not _has_table(insp, "ai_generation_jobs"):
        op.create_table(
            ...
            sa.Column(
                "reused", sa.Boolean(), nullable=False, server_default=sa.false()
            ),
        ...
        if not _has_column(insp, "ai_generation_jobs", "logs"):
            op.add_column(
                "ai_generation_jobs", sa.Column("logs", sa.JSON(), nullable=True)
            )

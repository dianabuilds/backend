...
def make_router() -> APIRouter:
    ...
    async def config(request: Request) -> dict[str, Any]:
        ...
        data: dict[str, Any] = {
            ...
            "jwt_secret_hint": sha256(
                s.auth_jwt_secret.get_secret_value().encode()
            ).hexdigest()[:10],
            "admin_key_present": bool(s.admin_api_key),
            "admin_key_len": (
                len(s.admin_api_key.get_secret_value()) if s.admin_api_key else 0
            ),
        }
        return data
    ...
    async def whoami(
        ...
        return {
            ...
            "claims": claims,
            "admin_key_present": bool(s.admin_api_key),
            "admin_key_match": bool(s.admin_api_key)
            and (admin_key == s.admin_api_key.get_secret_value()),
        }

...
class WorkerMetrics:
    ...
    def observe_job(self, *, cost_usd: float, prompt_tokens: int, completion_tokens: int) -> None:
    ...
    def observe_stage(self, stage: str, ms: float) -> None:
        ...
        self.stage_duration_sum_ms[s] = self.stage_duration_sum_ms.get(s, 0.0) + float(ms)

    def snapshot(self) -> Dict[str, Any]:
        out: Dict[str, Any] = {
            "jobs": dict(self.counters),
            "job_avg_ms": (self.duration_sum_ms / self.duration_count) if self.duration_count else 0.0,
            "cost_usd_total": self.cost_usd_total,
            "tokens": {"prompt": self.tokens_prompt_total, "completion": self.tokens_completion_total},
            "stages": {},
        }
    ...
    def prometheus(self) -> str:
        ...
        avg = (self.duration_sum_ms / self.duration_count) if self.duration_count else 0.0
        ...
        lines.append("# HELP ai_worker_cost_usd_total Total cost of generated jobs (USD)")
        ...
        lines.append(f'ai_worker_tokens_total{{type="prompt"}} {self.tokens_prompt_total}')
        lines.append(f'ai_worker_tokens_total{{type="completion"}} {self.tokens_completion_total}')
        # По-стадийные экспортируем как gauge_avg и counter

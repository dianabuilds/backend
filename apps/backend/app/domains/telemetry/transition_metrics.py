...
_route_latencies: deque[int] = deque(maxlen=10_000)
_route_lengths: dict[str, deque[int]] = defaultdict(lambda: deque(maxlen=10_000))
_preview_route_lengths: dict[str, deque[int]] = defaultdict(lambda: deque(maxlen=10_000))
_repeat_rates: deque[float] = deque(maxlen=10_000)
_novelty_rates: deque[float] = deque(maxlen=10_000)
_transition_counts: defaultdict[str, int] = defaultdict(int)
_preview_transition_counts: defaultdict[str, int] = defaultdict(int)
_no_route_counts: defaultdict[str, int] = defaultdict(int)
_preview_no_route_counts: defaultdict[str, int] = defaultdict(int)
_fallback_used_counts: defaultdict[str, int] = defaultdict(int)
_tag_entropies: dict[str, deque[float]] = defaultdict(lambda: deque(maxlen=10_000))


def record_latency(ms: int) -> None:
    with _transition_lock:
        _route_latencies.append(ms)


def record_route_length(profile_id: str | None, length: int, preview: bool = False) -> None:
...
def record_tag_entropy(profile_id: str | None, entropy: float) -> None:
    ws = profile_id or "unknown"
    with _transition_lock:

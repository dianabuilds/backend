...
async def run_ai_validation(db: AsyncSession, node_id: UUID) -> ValidationReport:
    ...
    Tries to validate node content using a large language model selected
    from :data:`VALIDATION_BUNDLE`. On any error, returns an empty report to
    gracefully fall back to local checks.
    """

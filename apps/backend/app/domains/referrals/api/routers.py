...
from app.security import ADMIN_AUTH_RESPONSES, auth_user, require_ws_editor, require_ws_guest
...
async def get_my_code(
    request: Request,
    workspace_id: int,
    _: Annotated[object, Depends(current_workspace)] = ...,
    ...
    code = await _svc(db).get_or_create_personal_code(db, workspace_id, current_user.id)
...
async def list_codes_admin(
    ...
    _: Annotated[object, Depends(current_workspace)] = ...,
    db: Annotated[AsyncSession, Depends(get_db)] = ...,
    __: Annotated[object, Depends(require_ws_editor)] = ...,
):
...
async def activate_code_admin(
    ...
    code = await repo.set_active(workspace_id, owner_user_id, True)
    if not code:
...
async def deactivate_code_admin(
    ...
    repo = ReferralsRepository(db)
    code = await repo.set_active(workspace_id, owner_user_id, False)
    if not code:
        raise HTTPException(status_code=404, detail="Code not found")
    # audit
    try:
...
async def export_events_admin(
    ...
    repo = ReferralsRepository(db)
    items0 = await repo.list_events(workspace_id=workspace_id, referrer_user_id=referrer_user_id, limit=limit, offset=offset)
    if date_from or date_to:
        items = []
        for it in items0:
            if date_from and it.occurred_at < date_from:
    ...
    def _stream() -> str:
        ...
        writer.writerow(["id", "workspace_id", "code", "referrer_user_id", "referee_user_id", "event_type", "occurred_at"])
        for it in items:
            writer.writerow([
                str(it.id),
                str(it.workspace_id),

...
class IReferralsRepository(Protocol):
    async def get_personal_code(
        self, owner_user_id: UUID
    ) -> ReferralCode | None:  # pragma: no cover
        ...

    async def create_personal_code(
        self, owner_user_id: UUID, code: str
    ) -> ReferralCode:  # pragma: no cover
        ...
    ...
    ) -> ReferralEvent | None:  # pragma: no cover
        ...
    ...
    async def list_codes(
        ...
        limit: int = 50,
        offset: int = 0,
    ) -> list[ReferralCode]:  # pragma: no cover
        ...

    async def list_events(
        self, referrer_user_id: UUID | None = None, limit: int = 50, offset: int = 0
    ) -> list[ReferralEvent]:  # pragma: no cover
        ...

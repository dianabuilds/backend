...
class ReferralsService:
    ...
    async def get_or_create_personal_code(self, db: AsyncSession, workspace_id: int, owner_user_id: UUID) -> ReferralCode:
        code = await self._repo.get_personal_code(workspace_id, owner_user_id)
        if code:
            return code
        ...
        if guess:
            ...
            if existing is None:
                return await self._repo.create_personal_code(workspace_id, owner_user_id, guess)
        # Fallback – generate unique random code
        # type: ignore[attr-defined]
        gen_code = await getattr(self._repo, "generate_unique_code")()  # provided by concrete repo
        return await self._repo.create_personal_code(workspace_id, owner_user_id, gen_code)
    ...
    async def process_signup_referral(
        ...
        evt = await self._repo.record_signup(workspace_id=int(code.workspace_id), code=code, referee_user_id=referee_user_id, meta=meta)
        if not evt:
            return False

        # Best-effort achievements trigger using referrer's default workspace UUID
        if code.owner_user_id:
            referrer = await db.get(User, code.owner_user_id)
            ws_uuid = getattr(referrer, "default_account_id", None)
            if ws_uuid:
                ach_svc = AchievementsService(AchievementsRepository(db))
                try:
                    await ach_svc.process_event(
                        db=db,
                        workspace_id=ws_uuid,
                        user_id=code.owner_user_id,
                        event="referral_signup",
                        payload=None,
                        preview=None,
                    )
                except Exception:
                    # Do not break signup flow due to achievements
                    pass
        return True

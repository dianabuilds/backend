...
class ConfigService:
    ...
    _cache: Optional[Tuple[float, int, RelevancePayload, datetime]] = None
    ...
    async def get_active_relevance(self) -> RelevanceGetOut:
        ...
        ver, payload, ts = row
        ConfigService._cache = (_now() + ConfigService._CACHE_TTL, int(ver), payload, ts)
        return RelevanceGetOut(version=int(ver), payload=payload, updated_at=ts)

    async def dry_run_relevance(self, payload: RelevancePayload, sample: List[str]) -> RelevanceDryRunOut:
    ...
    async def apply_relevance(self, payload: RelevancePayload, actor_id: Optional[str] = None) -> RelevanceApplyOut:
    ...
    async def rollback_relevance(self, to_version: int, actor_id: Optional[str] = None) -> RelevanceApplyOut:

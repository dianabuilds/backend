...
def register_comment_routes(router: APIRouter) -> None:
    ...
    async def list_node_comments(
        ...
        statuses = _normalize_comment_status_filter(status, include_deleted=include_deleted)
    ...
    async def set_comments_lock(
        ...
        await _emit_admin_activity(
            container,
            event="node.comments.locked.admin" if locked else "node.comments.unlocked.admin",
    ...
    @router.post("/{node_id}/comments/disable", summary="Enable or disable node comments")
    async def set_comments_disabled(
        ...
        await _emit_admin_activity(
            container,
            event="node.comments.disabled.admin" if disabled else "node.comments.enabled.admin",
            ...
            audit_action=(
                "product.nodes.comments.disable" if disabled else "product.nodes.comments.enable"
            ),
            audit_actor=actor_id or SYSTEM_ACTOR_ID,
            audit_resource_type="node",
            audit_resource_id=str(resolved_id),
    ...
    @router.post("/comments/{comment_id}/status", summary="Update comment status (admin)")
    async def update_comment_status_admin(
        ...
        await _emit_admin_activity(
            ...
            payload={"id": updated.id, "node_id": updated.node_id, "status": updated.status},

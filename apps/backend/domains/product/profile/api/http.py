...
def register_personal_routes(router: APIRouter) -> None:
    @router.get("/me")
...
def register_admin_routes(router: APIRouter) -> None:
    ...
    async def update_profile_admin(
        ...
        await log_profile_admin_action(
            ...
            action="profile.admin.api.update",
    ...
    async def legacy_update_username_route(
        ...
        # require_admin уже гарантирует привилегии; формируем subject с ролью admin
        ...
        await log_profile_admin_action(
            container,
            actor_id=resolve_actor_id(claims, request),
            action="profile.admin.api.legacy_username",
            target_user_id=user_id,
            request=request,
            after=payload,
            extra={"source": "profile_api", "legacy": True},
        )
        return payload
...
__all__ = ["make_router", "register_admin_routes", "register_personal_routes"]

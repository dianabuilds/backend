...
class SQLRepo(Repo):
    ...
    * Разработчик может постепенно заменять логику SQL/ORM по мере появления
    ...
    async def get(self, product_id: str) -> ProductDTO | None:  # type: ignore[override]
        ...
        return None if row is None else self._row_to_dto(cast(Mapping[str, Any], row))
    ...
    async def list_by_owner(
        ...
        return [self._row_to_dto(cast(Mapping[str, Any], row)) for row in rows]
...
def create_repo(dsn: str | None = None) -> SQLRepo:
    """Factory helper used by template wiring."""
    return SQLRepo(dsn=dsn)


__all__ = ["SQLRepo", "create_repo"]

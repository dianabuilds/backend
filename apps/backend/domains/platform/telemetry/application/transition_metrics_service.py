...
class TransitionMetrics:
    ...
    def observe_latency(
        self, tenant_id: str | None, mode: str | None, ms: float
    ) -> None:
    ...
    def observe_repeat_rate(
        self, tenant_id: str | None, mode: str | None, rate: float
    ) -> None:
        key = self._key(tenant_id, mode)
        with self._lock:
    ...
    def observe_entropy(
        self, tenant_id: str | None, mode: str | None, entropy: float
    ) -> None:
        key = self._key(tenant_id, mode)
        with self._lock:
    ...
    def prometheus(self) -> str:
        ...
        with self._lock:
            ...
            for key in keys:
                ...
                lines.append(
                    f'transition_latency_ms{{tenant_id="{ws}",mode="{mode}"}} {avg}'
                )
                ...
                lines.append(
                    f'no_route_ratio{{tenant_id="{ws}",mode="{mode}"}} {nr_ratio}'
                )
                lines.append(
                    f'fallback_ratio{{tenant_id="{ws}",mode="{mode}"}} {fb_ratio}'
                )
                ...
                lines.append(
                    f'novelty_rate{{tenant_id="{ws}",mode="{mode}"}} {nov_avg}'
                )
        return "\n".join(lines) + "\n"

...
from ..deps import get_actor_id, get_admin_contracts_use_case
...
def register(router: APIRouter) -> None:
    ...
    async def admin_upsert_contract(
        ...
        try:
            return await use_case.upsert_contract(payload=body, actor_id=actor_id)
    ...
    async def admin_delete_contract(
        ...
        try:
            return await use_case.delete_contract(
                id_or_slug=id_or_slug, actor_id=actor_id
            )
        except BillingUseCaseError as exc:
            raise HTTPException(status_code=exc.status_code, detail=exc.detail) from exc
    ...
    async def admin_add_contract_event(
        ...
        try:
            return await use_case.add_event(
                id_or_slug=id_or_slug, payload=body, actor_id=actor_id
            )
        except BillingUseCaseError as exc:
            raise HTTPException(status_code=exc.status_code, detail=exc.detail) from exc

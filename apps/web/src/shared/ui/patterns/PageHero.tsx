...
const VARIANT_CLASSNAMES: Record<PageHeroVariant, string> = {
  default:
    'border-gray-200/80 bg-gradient-to-br from-slate-50/90 via-white to-indigo-50/70 shadow-[0_20px_45px_-32px_rgba(79,70,229,0.45)] dark:border-dark-600/60 dark:from-dark-800/92 dark:via-dark-850 dark:to-dark-900/90 dark:shadow-[0_25px_60px_-45px_rgba(14,165,233,0.45)]',
  metrics:
...
function MetricsGrid({ metrics, variant }: { metrics: PageHeroMetric[]; variant: PageHeroVariant }) {
      ...
      {metrics.map((metric, index) => {
        ...
        return (
          <article
            key={key}
            className={clsx(
              'group relative overflow-hidden rounded-2xl border border-white/10 bg-white/10 p-5 backdrop-blur-md transition-transform hover:-translate-y-1 dark:border-white/5',
              variant === 'compact' && 'border-gray-200 bg-white/90 text-gray-900 shadow-sm dark:border-dark-700/60 dark:bg-dark-800/90 dark:text-gray-100',
              variant === 'default' && 'border-white/60 bg-white/80 shadow-[0_12px_40px_-32px_rgba(79,70,229,0.5)] dark:border-dark-700/40 dark:bg-dark-800/80'
            )}
          >
            <div className="flex items-start gap-3">
              {metric.icon ? <span className="mt-0.5 text-primary-500 dark:text-primary-300">{metric.icon}</span> : null}
              <div className="flex-1 space-y-2">
                <div className="text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">
                  {metric.label}
                </div>
                <div className="text-2xl font-semibold text-gray-900 dark:text-white lg:text-3xl">
                  {metric.value}
                </div>
                {metric.trend ? (
                  <div
                ...
                {metric.helper ? (
                  <div className="text-xs text-gray-500 dark:text-gray-300/80">{metric.helper}</div>
                ) : null}
              </div>
            </div>
          </article>
...
export function PageHero({
  ...
  return (
    <section className={containerClass} style={{ maxHeight: resolvedMaxHeight }}>
      <div className="relative z-[1] flex flex-col gap-6 lg:gap-8">
        {(eyebrow || (breadcrumbs && breadcrumbs.length > 0)) && (
          <div className={clsx('flex flex-wrap items-center gap-3 text-sm', align === 'center' ? 'justify-center' : 'justify-start')}>
            {eyebrow ? (
              <span className="rounded-full border border-white/60 bg-white/70 px-3 py-1 text-xs font-semibold uppercase tracking-[0.25em] text-primary-600 shadow-sm dark:border-white/10 dark:bg-white/10 dark:text-primary-200">
                {eyebrow}
              </span>
            ) : null}
        ...
        <div className={clsx('flex flex-col gap-6 lg:flex-row lg:items-end', align === 'center' ? 'lg:justify-center' : 'lg:justify-between')}>
          <div className={clsx('flex-1 space-y-4', headerAlignment)}>
            ...
            {description ? (
              <div className={clsx('max-w-3xl text-base sm:text-lg', variant === 'metrics' ? 'text-slate-200' : 'text-gray-600 dark:text-gray-200/85', align === 'center' && 'mx-auto')}>
                {description}
              </div>
            ) : null}
          </div>
        ...
        {filters ? (
          <div className={clsx('flex flex-wrap gap-3 rounded-2xl border border-white/40 bg-white/60 p-4 text-sm shadow-sm dark:border-white/10 dark:bg-white/5', align === 'center' ? 'justify-center' : 'justify-start')}>
            {filters}
          </div>
        ) : null}
        ...
        {metrics ? (
          <div className="space-y-4">
            {isMetricList(metrics) ? <MetricsGrid metrics={metrics} variant={variant} /> : metrics}
          </div>
        ) : null}
      ...
      <div className="pointer-events-none absolute -left-24 top-10 h-60 w-60 rounded-full bg-primary-400/10 blur-3xl dark:bg-primary-400/20" />
      <div className="pointer-events-none absolute -right-24 bottom-10 h-64 w-64 rounded-full bg-secondary/10 blur-3xl dark:bg-secondary/15" />
    </section>
  );
}
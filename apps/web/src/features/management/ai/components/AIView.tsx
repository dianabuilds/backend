...
} from '@shared/api/management/ai';
import { useAuth } from '@shared/auth';
import { FallbacksSection } from '../components/FallbacksSection';
...
import { useManagementAi } from '../hooks';
import { buildUsageRows, groupFallbacksByPrimary, groupFallbacksBySecondary } from '../utils';
...
export default function ManagementAI() {
  ...
  const totalErrors = usageRows.reduce<number>((acc, row: UsageRow) => acc + row.errors, 0);
  const totalCalls = usageRows.reduce<number>((acc, row: UsageRow) => acc + row.calls, 0);
      ...
      try {
        await saveManagementAiModel({
          ...
          params: {
            ...
            usage: {
              content: Boolean(form.params.usage?.content),
              quests: Boolean(form.params.usage?.quests),
              moderation: Boolean(form.params.usage?.moderation),
            },
  ...
  const handleSaveProvider = React.useCallback(
    async (form: ProviderFormState) => {
      ...
      try {
        await saveManagementAiProvider({
          slug: form.slug.trim(),
  ...
  const handleCreateFallback = React.useCallback(async () => {
    ...
    try {
      await createManagementAiFallback({
  ...
  const handleRemoveFallback = React.useCallback(
    async (id: string) => {
      ...
      try {
        await deleteManagementAiFallback(id);
        await loadAll({ silent: true });
      } finally {
  ...
  const handlePlayground = React.useCallback(async () => {
    ...
    try {
      ...
      const response = await runManagementAiPlayground({

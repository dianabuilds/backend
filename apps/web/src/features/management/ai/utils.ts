import type { FallbackRule, LLMSummary, UsageRow, MetricPoint } from './types';
...
export function buildUsageRows(metrics: LLMSummary | null): UsageRow[] {
  ...
  (metrics?.calls || []).forEach((row: MetricPoint) => {
  ...
  (metrics?.tokens_total || []).forEach((row: MetricPoint) => {
    const entry = ensure(row.provider, row.model);
    if (row.type === 'prompt') entry.promptTokens += row.total || 0;
    if (row.type === 'completion') entry.completionTokens += row.total || 0;
  });

  (metrics?.cost_usd_total || []).forEach((row: MetricPoint) => {
    const entry = ensure(row.provider, row.model);
    entry.costUsd += row.total_usd || 0;
  });

  (metrics?.latency_avg_ms || []).forEach((row: MetricPoint) => {
    const entry = ensure(row.provider, row.model);

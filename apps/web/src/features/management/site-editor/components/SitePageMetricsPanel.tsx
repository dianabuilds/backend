...
const STATUS_BADGE: Record<string, { label: string; color: 'success' | 'warning' | 'neutral' | 'error' }> = {
  ok: { label: 'Актуальны', color: 'success' },
  stale: { label: 'Просрочены', color: 'warning' },
...
function renderMetricCard(key: string, metric: SiteMetricValue): React.ReactElement | null {
  ...
  return (
    <MetricCard
      ...
      trendLabel={deltaLabel ? 'за выбранный период' : undefined}
      tone={config.tone}
    />
  );
}
...
export function SitePageMetricsPanel({
  ...
  const header = (
    <div className="flex flex-wrap items-center justify-between gap-3">
      <div className="space-y-1">
        {showTitle ? <div className="text-sm font-semibold text-gray-900 dark:text-white">Метрики страницы</div> : null}
        {metrics?.range ? (
          ...
          <div className="text-xs text-gray-500 dark:text-dark-300">Для выбранного периода пока нет данных.</div>
        )}
      </div>
    ...
    <>
      ...
      {!loading && metrics && Object.keys(metrics.metrics).length === 0 && metrics.alerts.length === 0 ? (
        <div className="rounded-xl border border-dashed border-gray-200 px-3 py-2 text-xs text-gray-500 dark:border-dark-600 dark:text-dark-300">
          Для выбранного периода нет данных.
        </div>
      ) : null}

      {metrics && Object.keys(metrics.metrics).length > 0 ? (
        <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
          {Object.entries(metrics.metrics).map(([key, value]) => (value ? renderMetricCard(key, value) : null))}
        </div>
      ) : null}
        ...
        <div className="space-y-2">
          {metrics.alerts.map((alert) => (
            <div
              ...
              className={`flex items-start gap-2 rounded-xl border px-3 py-2 text-xs ${ALERT_COLORS[alert.severity]}`}
            >
              <AlertTriangle className="mt-0.5 h-4 w-4" />
              <div className="space-y-1">
  ...
  if (variant === 'card') {
    return (
      <Card className={`space-y-4 p-5 ${className}`.trim()}>
        {header}
        {body}
      </Card>
    );

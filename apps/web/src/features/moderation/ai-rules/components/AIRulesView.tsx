...
export default function ModerationAIRules() {
  ...
  const toggle = React.useCallback(async (rule: ModerationAIRule) => {
    try {
  ...
  const remove = React.useCallback(async (rule: ModerationAIRule) => {
    try {
  ...
  const openDrawer = React.useCallback((rule: ModerationAIRule) => {
...
<div className="relative overflow-x-auto rounded-lg border border-gray-200 dark:border-dark-500">
  <UITable preset="surface" className="min-w-[840px]" hover>
    <UITable.THead>
      <UITable.TR>
      ...
      {filteredItems.length === 0 ? (
        ...
        filteredItems.map((rule) => (
          <UITable.TR key={rule.id}>
            ...
            <UITable.TD className="text-sm text-gray-600 dark:text-dark-200">{rule.description || '—'}</UITable.TD>
            <UITable.TD className="text-sm text-gray-600 dark:text-dark-200">{rule.default_action || '—'}</UITable.TD>

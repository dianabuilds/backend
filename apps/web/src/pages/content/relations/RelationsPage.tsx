        ...
        <div className="mt-4 overflow-x-auto">
          <table className="w-full text-left text-sm">
            ...
            <tbody className="divide-y divide-gray-100 dark:divide-dark-700">
              {loadingRelations
                ? Array.from({ length: 4 }).map((_, index) => (
                    <tr key={`relation-skeleton-${index}`}>
                ...
                : relations.map((rel) => (
                    ...
                    >
                      <td className="px-4 py-3 text-sm text-gray-700 dark:text-dark-100">
                      ...
                      <td className="px-4 py-3 text-sm text-gray-700 dark:text-dark-200">
                        {rel.score != null && !Number.isNaN(rel.score) ? rel.score.toFixed(2) : '0.00'}
                      </td>
                      <td className="px-4 py-3 text-xs text-gray-400 dark:text-dark-300">
                        {humanizeTimestamp(rel.updated_at)}
                      </td>
                    </tr>
                  ))}
              {!loadingRelations && relations.length === 0 && (
                <tr>
                  <td
                    className="px-4 py-6 text-center text-sm text-gray-500 dark:text-dark-200"
                    colSpan={4}
                  >
                    No cached relations for this strategy yet. Refresh the snapshot or trigger the
                    recompute job from the backend to populate this table.
                  </td>
                </tr>
              )}
            </tbody>

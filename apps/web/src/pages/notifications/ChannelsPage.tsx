...
type ChannelStatus = 'required' | 'recommended' | 'optional';
...
const STATUS_META: Record<ChannelStatus, { label: string; color: 'warning' | 'info' | 'neutral' }> = {
...
export default function ChannelsPage(): React.ReactElement {
  ...
  const fetchOverview = React.useCallback(
    async (mode: 'initial' | 'refresh' = 'initial') => {
      ...
      setError(null);
      try {
        const response = await apiGet<OverviewResponse>('/v1/me/settings/notifications/preferences');
      ...
      <NotificationSurface className="space-y-6 p-6">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <Button variant="outlined" color="neutral" onClick={handleRefresh} disabled={refreshing || loading}>
            ...
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              <div className="rounded-2xl border border-white/60 bg-white/80 p-4 shadow-sm">
                ...
                <div className="mt-2 text-2xl font-semibold text-gray-900">
                  ...
                  <span className="text-base font-medium text-gray-400"> / {summary?.total_channels ?? channels.length}</span>
                </div>
              ...
              <div className="rounded-2xl border border-white/60 bg-white/80 p-4 shadow-sm">
                ...
                <div className="mt-2 text-2xl font-semibold text-gray-900">
                  {summary?.email_digest ? summary.email_digest : 'instant'}
                </div>
              ...
              <div className="rounded-2xl border border-white/60 bg-white/80 p-4 shadow-sm">
                ...
                <div className="mt-2 text-lg font-semibold text-gray-900">{formatDateTime(summary?.updated_at)}</div>
                    ...
                    <div className="grid gap-3 md:grid-cols-2">
                      {topic.channels.map((channel) => {
                        ...
                        return (
                          <div
                            key={channel.key}
                            className="space-y-2 rounded-xl border border-indigo-100/60 bg-white/90 p-3 shadow-sm"
                          >
                            ...
                            {channel.locked && (
                              <Badge color="warning" variant="soft">Locked</Badge>
                            )}

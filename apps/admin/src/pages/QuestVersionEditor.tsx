...
export default function QuestVersionEditor() {
  const { id: questId, versionId: id } = useParams<{ id: string; versionId: string }>();
  ...
  const startEditNode = (k: string) => {
    ...
    if (content && typeof content === 'object') contentData = content as OutputData;
    else contentData = { time: Date.now(), blocks: [], version: '2.30.7' };
  ...
  const filteredEdges = useMemo(() => {
    ...
    return graph.edges.filter(
      (e) => e.from_node_key.toLowerCase().includes(q) || e.to_node_key.toLowerCase().includes(q),
    );
  }, [graph, edgesQuery]);
  ...
  const deleteNode = async (key: string) => {
    if (!graph) return;
    const ok = await confirmDialog(`Delete node "${key}"? All connected edges will be removed.`);
    if (!ok) return;
      ...
      <PageLayout
        ...
        actions={
          <div className="flex items-center gap-3">
            ...
            <button className="px-3 py-1 rounded border" onClick={() => setShowSim((v) => !v)}>
              {showSim ? 'Close Sim' : 'Simulate'}
            </button>
            <button className="px-3 py-1 rounded bg-green-600 text-white" onClick={onPublish}>
              Publish
            </button>
          </div>
        }
                      ...
                      <div className="md:col-span-2 space-y-2">
                        ...
                        {meta.tags && meta.tags.length > 0 && (
                          <div className="flex flex-wrap gap-2">
                            {meta.tags.map((t) => (
                              <span key={t} className="px-2 py-1 text-sm rounded bg-gray-200">
                                {t}
                              </span>
                            ))}
                          </div>
            ...
            <div className="col-span-2 space-y-4">
              <CollapsibleSection title="Nodes" defaultOpen={true}>
                ...
                <div className={nodesMultiColumn ? 'grid md:grid-cols-2 lg:grid-cols-3 gap-2' : ''}>
                  ...
                  />
                  <button className="px-3 py-1 rounded border" onClick={() => setPickerOpen(true)}>
                    Pick
                  </button>
                  <select
                    className="border rounded px-2 py-1"
                    ...
                    onChange={(e) => setNodeType(e.target.value as 'start' | 'normal' | 'end')}
                    title={
                  ...
                  <button
                    ...
                    title={
                      graph.nodes.find((n) => n.key === slugify(nodeKey)) ? 'Duplicate key' : ''
                    }
                  >
                    Add
                  </button>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    <div>
                      ...
                      <ul className="list-disc pl-5 text-red-700">
                        ...
                        {clientErrors.length === 0 && <li className="text-gray-500">None</li>}
                      </ul>
                    </div>
                    <div>
                      <div className="font-semibold mb-1">Warnings</div>
                      <ul className="list-disc pl-5 text-yellow-700">
                        ...
                        {clientWarnings.length === 0 && <li className="text-gray-500">None</li>}
                      </ul>
                    </div>
                  </div>
                )}
              ...
              <div className="rounded border" style={{ maxHeight: '70vh', overflow: 'auto' }}>
                <GraphCanvas
                  nodes={graph.nodes.map((n) => ({
                    key: n.key,
                    title: n.title,
                    type: (n.type as 'start' | 'normal' | 'end' | undefined) ?? 'normal',
                  }))}
                  edges={graph.edges}
                  ...
                  onCreateEdge={(from, to) => {
                    setGraph((g) => {
                      if (!g) return g;
                      if (from === to) return g;
                      if (g.edges.some((e) => e.from_node_key === from && e.to_node_key === to))
                        return g;
                      return {
                        ...g,
                        edges: [
            ...
            <div className="bg-white w-full max-w-[95vw] md:max-w-7xl max-h-[92vh] flex flex-col">
              <ContentEditor
                ...
                general={{
                  ...
                  onTitleChange: (v) => setEditorNode((p) => (p ? { ...p, title: v } : p)),
                  onSlugChange: (v: string) => setEditorNode((p) => (p ? { ...p, slug: v } : p)),
                  ...
                  onCoverChange: (url) => setEditorNode((p) => (p ? { ...p, coverUrl: url } : p)),
                }}
                content={{
                  initial: editorNode.contentData,
                  onSave: (d) => setEditorNode((p) => (p ? { ...p, contentData: d } : p)),
                }}
              />
            </div>
          </div>

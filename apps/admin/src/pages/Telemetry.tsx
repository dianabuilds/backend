...
type RumSummary = {
  ...
  navigation_avg: { ttfb_ms: number | null; dom_content_loaded_ms: number | null; load_event_ms: number | null };
};

export default function Telemetry() {
  const qc = useQueryClient();

  const { data: summary, isFetching: sFetching, error: sError } = useQuery({
    queryKey: ["telemetry", "summary"],
    queryFn: async () => (await api.get<RumSummary>("/admin/telemetry/rum/summary")).data!,
  ...
  const { data: events, isFetching: eFetching, error: eError } = useQuery({
    ...
    queryFn: async () => (await api.get<RumEvent[]>("/admin/telemetry/rum?limit=200")).data || [],
    refetchInterval: 5000,
    refetchOnWindowFocus: true,
    staleTime: 2000,
  });
    ...
    <div className="p-4 space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="rounded border p-3">
          ...
          {sFetching ? <div className="text-xs text-gray-500 mt-1">Загрузка…</div> : null}
          {sError ? <div className="text-xs text-red-600 mt-1">Ошибка: {(sError as any)?.message}</div> : null}
          {summary ? (
            <div className="text-sm mt-2 space-y-1">
              ...
              <div>DCL avg: {summary.navigation_avg.dom_content_loaded_ms ?? "-"} ms</div>
              <div>Load avg: {summary.navigation_avg.load_event_ms ?? "-"} ms</div>
              <div className="mt-2">
                <div className="text-gray-500">Счётчики по событиям</div>
                <ul className="list-disc pl-5">
                  {Object.keys(counts).length === 0 ? <li className="text-gray-500">нет данных</li> : null}
        ...
        <div className="md:col-span-2 rounded border p-3">
          ...
          {eError ? <div className="text-xs text-red-600 mt-1">Ошибка: {(eError as any)?.message}</div> : null}
              ...
              <tbody>
                {(events || []).map((ev, idx) => (
                  <tr key={idx} className="border-t">
                    <td className="px-2 py-1">{ev.ts ? new Date(ev.ts).toLocaleTimeString() : "-"}</td>
                ...
                {(events || []).length === 0 ? (
                  <tr><td colSpan={4} className="px-2 py-3 text-sm text-gray-500">Пока нет событий</td></tr>
                ) : null}
              </tbody>
            </table>
          </div>

...
function SettingsTabs() {
    ...
    const [modelTest, setModelTest] = useState<{ loading: boolean; result?: { ok: boolean; status: number; latency_ms?: number; excerpt?: string } | null }>({ loading: false, result: null });
    const [priceUnit, setPriceUnit] = useState<'per_1k' | 'per_1m'>('per_1k');
    const [, setModelErrors] = useState<{ provider_id?: string; code?: string; input_price?: string; output_price?: string }>({});
    const [importDraft, setImportDraft] = useState<{ provider_id?: string; items?: any[] } | null>(null);
    ...
    const saveManifest = async () => {
        ...
        try {
            ...
            await api.put(`/admin/ai/system/providers/${encodeURIComponent(providerDraft.id)}/manifest`, parsed);
    ...
    const openSecretsEditor = async (providerId: string) => {
        const res = await api.get(`/admin/ai/system/providers/${encodeURIComponent(providerId)}/manifest`);
    ...
    const saveSecrets = async () => {
        ...
        try {
            ...
            await api.post(`/admin/ai/system/providers/${encodeURIComponent(pid as string)}/secrets`, secretsDraft);
            setProviderDraft(null);
        } catch (e) {
            alert(e instanceof Error ? e.message : String(e));
        }
    ...
    const saveModel = async () => {
        ...
        const errs: { provider_id?: string; code?: string; input_price?: string; output_price?: string } = {};
        ...
        if (inV !== undefined || outV !== undefined) {
            ...
            if (priceUnit === 'per_1m') {
                if (payload.pricing.input_per_1k !== undefined) payload.pricing.input_per_1k = payload.pricing.input_per_1k / 1000;
                if (payload.pricing.output_per_1k !== undefined) payload.pricing.output_per_1k = payload.pricing.output_per_1k / 1000;
            }
        }

        if ((modelDraft as any).id) {
            await api.put(`/admin/ai/system/models/${encodeURIComponent((modelDraft as any).id as string)}` as string, payload);
        } else {
            ...
            render: (p) => {
                ...
                return (
                    <div className="flex items-center gap-2">
                        <span className="font-mono text-xs" title={text}>{visible ? text : masked}</span>
                        {text && (
                            <button
                                ...
                                onClick={() =>
                                    setShowCodes((s) => {
                                        ...
                                        if (visible) copy.delete(id); else copy.add(id);
                                        return copy;
                                    })
                                }
                            >
        ...
        },
        {
            ...
            render: (p) =>
                defaults.data?.provider_id === p.id ? (
                    ...
                    <button className="text-green-600 flex items-center gap-1" onClick={() => setDefaultProvider(p.id)}>
        ...
        },
        {
            ...
            render: (p) => (
                <div className="flex gap-2">
                    <button className="text-blue-600" onClick={() => setProviderDraft(p)} title="Edit connection">
                        <Pencil className="w-4 h-4"/>
                    </button>
                    <button className="text-purple-600" onClick={() => openSecretsEditor(p.id)} title="Set credentials">
            ...
            render: (m) => {
                ...
                return (
                    <div className="flex flex-col">
                        <span>{name}</span>
                        {masked && (
                            <span className="text-xs text-gray-500 font-mono">{masked}</span>
                        )}
                    </div>
                );
            },
            ...
            render: (m) => {
                ...
                return (
                    <div className="flex items-center gap-2">
                        ...
                        <span className={enabled ? 'text-green-600' : status === 'fail' ? 'text-red-600' : 'text-gray-500'}>
        ...
        },
        {
            ...
            render: (m) =>
                defaults.data?.model_id === m.id ? (
                    ...
                    <button className="text-blue-600 flex items-center gap-1" onClick={() => setDefaultModel(m.id)}>
                        <CheckCircle2 className="w-4 h-4" /> Set
                    </button>
                ),
        },
        {
            ...
            render: (m) => (
                <div className="flex gap-2">
                    ...
                    <button className="text-green-600" onClick={() => testModelConnection(String(m.id))} title="Test">
                                            ...
                                            <div className="space-y-3">
                                                    ...
                                                    <div key={k} className="grid grid-cols-1 md:grid-cols-5 gap-2 items-end">
                                                        ...
                                                        <div className="md:col-span-5 flex justify-between">
                                                            <button
                                                                ...
                                                                onClick={() => setSecretsDraft((s) => ({ ...(s || {}), ...(s['api_key']===undefined ? { api_key: '' } : { token: '' }) }))}
                                                            >
                                                ...
                                                <div className="flex justify-end gap-2 pt-1">
                                                    <button className="px-3 py-1 rounded bg-gray-200"
                                                            onClick={() => setProviderDraft(null)}>Cancel
                                                    </button>
                                                    <button className="px-3 py-1 rounded bg-blue-600 text-white"
                                                            onClick={saveSecrets}>Save
                                                    </button>
                                                </div>
                                            </div>
                                        ) : (
                                            <div className="space-y-2">
                                                ...
                                                <div className="flex flex-col gap-1">
                                                    ...
                                                    <div className="relative">
                                                        ...
                                                        >
                                                            {revealProviderCode ? <EyeOff className="w-4 h-4"/> :
                                                                <Eye className="w-4 h-4"/>}
                                                        </button>
                                                    </div>
                                                    <div className="text-xs text-gray-500">If no credential is set via Secrets, this value will be used as Bearer token.</div>
                                                </div>
                                                <div className="flex flex-col gap-1">
                                                    ...
                                                    />
                                                    <div className="text-xs text-gray-500">Tip: use domain only (e.g., https://api.aimlapi.com). Path (/v1/...) is handled automatically.</div>
                                                </div>
                        ...
                        <div className="space-y-4">
                                    ...
                                    <div className="flex items-center gap-2">
                                        ...
                                        >
                                            ...
                                            {providers.data?.map((p: Provider) => (
                                                <option key={p.id} value={p.id}>{(p.name || (p.base_url ? (() => {
                                                    try {
                                                        return new URL(p.base_url).host
                                                    } catch {
                                                        return String(p.base_url)
                                                    }
                                                })() : (String(p.code || '').slice(0, 8) + '�' + String(p.code || '').slice(-4))))}</option>
                                            ))}
                                        </select>
                                        <button
                                            className="flex items-center gap-1 px-2 py-1 rounded bg-blue-600 text-white"
                                ...
                                <div className="space-y-2">
                                    ...
                                    <div className="flex flex-col gap-1">
                                        ...
                                        <div className="flex flex-wrap gap-2">
                                            {['chat', 'tools', 'json_mode', 'vision', 'long_context', 'stream', 'embed', 'rerank'].map((cap) => {
                                                const active = (modelDraft?.capabilities as string[] | undefined)?.includes(cap);
                                                return (
                                                    <label key={cap}
                                                        ...
                                                        <input
                                                            ...
                                                            onChange={(e) =>
                                                                setModelDraft((s) => {
                                                                    const set = new Set<string>(Array.isArray(s?.capabilities) ? (s!.capabilities as string[]) : []);
                                                                    if (e.target.checked) set.add(cap); else set.delete(cap);
                                                                    return {
                                                                        ...(s || {}),
                                    ...
                                    <div className="flex items-center gap-3 text-sm">
                                        ...
                                        <select className="border rounded px-2 py-1" value={priceUnit} onChange={(e) => setPriceUnit(e.target.value as 'per_1k' | 'per_1m')}>
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
                                        <div className="flex flex-col gap-1">
                                            <label className="text-sm text-gray-600 min-h-10">Input price ({priceUnit === 'per_1k' ? 'per 1k' : 'per 1M'}) (USD)</label>
                                            <input
                                                className="border rounded px-2 py-1"
                                                inputMode="decimal"
                                                pattern="[0-9]*[.,]?[0-9]*"
                                                ...
                                                onChange={(e) => {
                                                    ...
                                                    if (raw === '') {
                                                        setModelDraft((s) => ({ ...(s || {} as any), pricing: { ...((s as any)?.pricing||{}), input_per_1k: undefined } }));
                                                        return;
                                                    }
                                                    if (!/^\d*(?:\.\d*)?$/.test(raw)) return;
                                                    const num = Number(raw);
                                                    if (!Number.isFinite(num)) return;
                                                    setModelDraft((s) => ({ ...(s || {} as any), pricing: { ...((s as any)?.pricing||{}), input_per_1k: num } }));
                                                }}
                                            />
                                        </div>
                                        <div className="flex flex-col gap-1">
                                            <label className="text-sm text-gray-600 min-h-10">Output price   ({priceUnit === 'per_1k' ? 'per 1k' : 'per 1M'}) (USD)</label>
                                            <input
                                                className="border rounded px-2 py-1"
                                                inputMode="decimal"
                                                pattern="[0-9]*[.,]?[0-9]*"
                                                ...
                                                onChange={(e) => {
                                                    const raw = e.target.value.replace(',', '.');
                                                    if (raw === '') {
                                                        setModelDraft((s) => ({ ...(s || {} as any), pricing: { ...((s as any)?.pricing||{}), output_per_1k: undefined } }));
                                                        return;
                                                    }
                                                    if (!/^\d*(?:\.\d*)?$/.test(raw)) return;
                                                    const num = Number(raw);
                                                    if (!Number.isFinite(num)) return;
                                                    setModelDraft((s) => ({ ...(s || {} as any), pricing: { ...((s as any)?.pricing||{}), output_per_1k: num } }));
                                                }}
                                            />
                                        </div>
                                        <div className="flex flex-col gap-1">
                                    ...
                                    <div className="text-xs text-gray-500">Leave pricing empty to track tokens only.</div>
                                    {modelDraft?.id ? (
                                        <div className="rounded border p-2 bg-gray-50 text-xs text-gray-700 mt-2">
                                                ...
                                                <div className="flex gap-2">
                                                    <button
                                                        ...
                                                        onClick={async () => {
                                                            ...
                                                            try {
                                                                const res = await api.post(`/admin/ai/system/models/${encodeURIComponent(String(modelDraft.id))}/test`, { prompt: 'ping' });
                                                                setModelTest({ loading: false, result: res.data as any });
                                                            } catch (e) {
                                                                setModelTest({ loading: false, result: { ok: false, status: 0, excerpt: e instanceof Error ? e.message : String(e) } as any });
                                                            }
                                                        }}
                                                    >
                                            ...
                                            {modelTest.result ? (
                                                <div className="mt-1">
                                                    <div>Status: {modelTest.result.ok ? 'OK' : 'Fail'} ({modelTest.result.status}){modelTest.result.latency_ms !== undefined ? ` · ${modelTest.result.latency_ms} ms` : ''}</div>
                                                    {modelTest.result.excerpt ? (
                                                        <pre className="mt-1 max-h-40 overflow-auto whitespace-pre-wrap">{modelTest.result.excerpt}</pre>
                                                    ) : null}
                                                </div>
                                            ) : null}
                                        </div>
                                    ...
                                    <div className="flex justify-end gap-2 pt-1">
                                        <button className="px-3 py-1 rounded bg-gray-200"
                                                onClick={() => setModelDraft(null)}>Cancel
                                        </button>
                                        <button className="px-3 py-1 rounded bg-blue-600 text-white"
                            ...
                            >
                                <div className="space-y-3">
                                    <div className="flex flex-col gap-1">
                                        ...
                                        <select
                                            ...
                                            onChange={async (e) => {
                                                ...
                                                const res = await api.get(`/admin/ai/system/providers/${encodeURIComponent(pid)}/manifest`);
                                                const items = Array.isArray((res.data as any)?.models) ? (res.data as any).models : [];
                                                setImportDraft({
                                                    provider_id: pid,
                                    ...
                                    <div className="max-h-80 overflow-auto border rounded">
                                        {((importDraft?.items as any[]) || []).map((m: any, i: number) => (
                                            <label key={`${m.id}-${i}`}
                                                ...
                                                <input type="checkbox" checked={!!m.__selected}
                                                       onChange={(e) => setImportDraft((s) => {
                                                           const copy: any = {...(s || {})};
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
                                        <div className="flex flex-col gap-1">
                                            <label className="text-sm text-gray-600">input_per_1k (USD,
                                                optional)</label>
                                            <input id="imp-in" className="border rounded px-2 py-1"/>
                                        </div>
                                        <div className="flex flex-col gap-1">
                                            <label className="text-sm text-gray-600">output_per_1k (USD,
                                                optional)</label>
                                            <input id="imp-out" className="border rounded px-2 py-1"/>
                                        </div>
                                        <div className="flex flex-col gap-1">
                                            <label className="text-sm text-gray-600 min-h-10">currency</label>
                                            <input id="imp-cur" className="border rounded px-2 py-1"
                                                   defaultValue="USD"/>
                                        </div>
                                    </div>
                                    <button
                                        ...
                                        onClick={async () => {
                                            ...
                                            if (!pid) return;
                                            const inV = (document.getElementById('imp-in') as HTMLInputElement | null)?.value;
                                            const outV = (document.getElementById('imp-out') as HTMLInputElement | null)?.value;
                                            const curV = (document.getElementById('imp-cur') as HTMLInputElement | null)?.value || 'USD';
                                            const pricing = (inV || outV) ? {
...
function RoutingProfilesTab() {
    ...
    const rows = useMemo(() => ((profiles.data as any[]) || []).filter((p: any) => (p.name || '').toLowerCase().includes(search.toLowerCase())), [profiles.data, search]);
    const capabilityOptions = useMemo(() => {
        ...
        const known = ['chat', 'tools', 'json_mode', 'vision', 'long_context', 'stream', 'embed', 'rerank'];
    ...
    const save = async () => {
        if (!draft) return;
        const body = {name: draft.name || 'default', enabled: !!draft.enabled, rules: draft.rules || []};
                ...
                <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                        <div className="relative">
                            ...
                            <input className="border rounded pl-6 pr-2 py-1" placeholder="Search..." value={search}
                                   onChange={(e) => setSearch(e.target.value)}/>
                        </div>
                        <button className="flex items-center gap-1 px-2 py-1 rounded bg-blue-600 text-white"
                                onClick={() => setDraft({enabled: true, rules: []})}>
                            <Plus className="w-4 h-4"/> Add
                        </button>
                    </div>
                </div>
                <DataTable columns={columns} rows={rows} rowKey={(r) => String(r.id)} rowClassName="odd:bg-gray-50"/>
            </div>
            <Slideover open={!!draft} title={draft?.id ? 'Edit profile' : 'New profile'} onClose={() => setDraft(null)}>
                <div className="space-y-4">
                    <div className="flex flex-col gap-1">
                        <label className="text-sm text-gray-600">name</label>
                        <input className="border rounded px-2 py-1 w-full" value={draft?.name || ''}
                               onChange={(e) => setDraft((s: any) => ({...(s || {}), name: e.target.value}))}/>
                    </div>
                    <div className="flex items-center gap-2">
                        <input id="prof-enabled" type="checkbox" checked={!!draft?.enabled}
                               onChange={(e) => setDraft((s: any) => ({...(s || {}), enabled: e.target.checked}))}/>
                        <label htmlFor="prof-enabled" className="text-sm text-gray-600">enabled</label>
                    </div>
                    ...
                    <div className="space-y-3">
                        ...
                        {((draft?.rules as any[]) || []).map((rule: any, idx: number) => {
                            ...
                            const providerModels = ((models.data as any[]) || []).filter((m) => !providerId || String(m.provider_id) === providerId);
                            return (
                                                    ...
                                                    <label key={cap}
                                                        ...
                                                        <input
                                                            ...
                                                            onChange={(e) =>
                                                                setDraft((s: any) => {
                                                                    const copy = {...(s || {})};
                                                                    const arr = new Set<string>(copy.rules[idx].selector?.capabilities || []);
                                        ...
                                        <div className="flex flex-col gap-1">
                                            ...
                                            >
                                                ...
                                                {((providers.data as any[]) || []).map((p) => (
                                                    <option key={String(p.id)}
                                                            value={String(p.id)}>{p.code || p.name}</option>
                                                ))}
                                            </select>
                                        </div>
                                        <div className="flex flex-col gap-1">
                                            ...
                                            >
                                                ...
                                                {providerModels.map((m) => (
                                                    <option key={String(m.id)}
                                                            value={String(m.id)}>{m.name || m.code}</option>
                                                ))}
                                            </select>
                                        </div>
                                    </div>
                                        ...
                                        {(rule.route?.fallback || []).map((fb: any, j: number) => (
                                            <div key={j} className="grid grid-cols-1 md:grid-cols-3 gap-2 items-center">
                                                ...
                                                >
                                                    ...
                                                    {(((models.data as any[]) || []).filter((m) => !fb.provider_id || String(m.provider_id) === String(fb.provider_id))).map((m) => (
                                                        <option key={String(m.id)}
                                                                value={String(m.id)}>{m.name || m.code}</option>
                                                    ))}
                                                </select>
                    ...
                    <div className="flex gap-2">
                        ...
                        <button className="px-3 py-1 rounded bg-gray-200" onClick={() => setDraft(null)}>Cancel</button>
                    </div>
                </div>
            </Slideover>
        </div>
...
function PresetsTab() {
    ...
    const [search, setSearch] = useState('');
    const rows = useMemo(() => ((presets.data as any[]) || []).filter((p: any) => (p.name || '').toLowerCase().includes(search.toLowerCase())), [presets.data, search]);

    const save = async () => {
        if (!draft) return;
        const body = {name: draft.name || 'preset', task: draft.task || 'chat', params: draft.params || {}};
    ...
    const columns: Column<any>[] = [
        {key: 'name', title: 'Name'},
        {key: 'task', title: 'Task'},
        {key: 'params', title: 'Params', accessor: (r) => (JSON.stringify(r.params)?.slice(0, 60) || '') + '...'},
        {
            key: 'actions',
            title: 'Actions',
            render: (r) => (
    ...
    return (
        <div className="space-y-4">
            ...
            <Slideover open={!!draft} title={draft?.id ? 'Edit preset' : 'New preset'} onClose={() => setDraft(null)}>
                <div className="space-y-2">
                    <div className="flex flex-col gap-1">
                        <label className="text-sm text-gray-600">name</label>
                        <input className="border rounded px-2 py-1 w-full" value={draft?.name || ''}
                    ...
                    <div className="flex flex-col gap-1">
                        <label className="text-sm text-gray-600">task</label>
                        <select className="border rounded px-2 py-1 w-full" value={draft?.task || 'chat'}
                        ...
                        {(!draft?.task || draft.task === 'chat') && (
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    />
                                    <label htmlFor="json-mode" className="text-sm text-gray-600">json_mode</label>
                                </div>
                                <div className="flex items-center gap-2">
                                    <input
                                        id="stream"
                                    ...
                                    />
                                    <label htmlFor="stream" className="text-sm text-gray-600">stream</label>
                                </div>
                            </div>
                        )}
                        ...
                        {draft?.task === 'rerank' && (
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    />
                                    <label htmlFor="return-docs"
                                           className="text-sm text-gray-600">return_documents</label>
                                </div>
                            </div>
                        )}
                    </div>

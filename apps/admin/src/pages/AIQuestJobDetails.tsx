  ...
  return (
    <div className="p-4 space-y-4">
      <div className="flex items-center gap-2">
        <button className="text-sm text-blue-600 hover:underline" onClick={() => nav(-1)}>
          &larr; Назад
        </button>
            ...
            <div className="rounded border p-3">
              ...
              <div className="mt-1 text-sm">
                ...
                <div>
                  ...
                  <StatusBadge status={summary?.status || '-'} reused={summary?.reused} />
                </div>
                <div>
                  <span className="text-gray-500">Провайдер:</span> {summary?.provider}{' '}
                  <span className="ml-2 text-gray-500">Модель:</span> {summary?.model}
                </div>
                <div>
                  <span className="text-gray-500">Создана:</span> {data.job.created_at || '-'}{' '}
                  <span className="ml-2 text-gray-500">Начата:</span> {data.job.started_at || '-'}{' '}
                ...
                <div>
                  ...
                  <span className="ml-2 text-gray-500">Токены:</span> {summary?.tokens ?? '-'}
                </div>
                {data.job.error ? (
                  <div className="text-red-600 break-words mt-2">Ошибка: {data.job.error}</div>
                ) : null}
              </div>
                  ...
                  {validation ? (
                    <div className="text-sm">
                      <div>
                        ...
                        <span
                          className={
                            validation.report.errors > 0 ? 'text-red-600' : 'text-green-600'
                          }
                        >
                          {validation.report.errors}
                        </span>
                        , предупреждений:{' '}
                        <span className="text-yellow-600">{validation.report.warnings}</span>
                      </div>
                      <details className="mt-2">
                        ...
                        <ul className="mt-2 list-disc pl-5 space-y-1">
                          {validation.report.items.map((it, idx) => (
                            <li
                              key={idx}
                              className={it.level === 'error' ? 'text-red-600' : 'text-yellow-700'}
                            >
              ...
              <table className="min-w-full text-sm">
                ...
                <tbody>
                  ...
                  {data.stage_logs.length === 0 ? (
                    <tr>
                      <td className="px-2 py-3 text-gray-500 text-sm" colSpan={6}>
...
function StatusBadge({ status, reused }: { status: string; reused?: boolean }) {
  ...
  return (
    <span className="inline-flex items-center gap-2">
      <span className={`px-2 py-0.5 rounded text-xs ${map[status] || 'bg-gray-200'}`}>
        {status}
      </span>
      {reused ? (

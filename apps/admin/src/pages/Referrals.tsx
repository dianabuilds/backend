...
export default function Referrals() {
  ...
  const [eventsReferrer, setEventsReferrer] = useState('');
  ...
  const confirmReason = async () => {
    try {
      if (pendingAction === 'activate') {
        await activateReferralCode(pendingOwner, reasonText || undefined);
        addToast({ title: 'Code activated', variant: 'success' });
      } else {
        await deactivateReferralCode(pendingOwner, reasonText || undefined);
      ...
      addToast({
        title: pendingAction === 'activate' ? 'Activation failed' : 'Deactivation failed',
        variant: 'error',
      });
    }
  };
      ...
      actions={
        <div className="flex gap-2">
          <Button
            ...
            variant={tab === 'codes' ? 'primary' : 'secondary'}
          >
            Codes
          </Button>
          <Button
        ...
        {tab === 'codes' ? (
          ...
          <>
            <div>
              ...
              <TextInput
                value={eventsReferrer}
                onChange={(e) => setEventsReferrer(e.target.value)}
                className="w-80"
              />
            </div>
            <div>
            ...
            <div className="flex items-center gap-2">
              ...
              <TextInput
                ...
                value={eventsLimit}
                onChange={(e) =>
                  setEventsLimit(Math.max(1, Math.min(1000, Number(e.target.value) || 1)))
                }
                className="w-20"
              />
              <Button
                disabled={eventsPage === 0 || loading}
                onClick={() => setEventsPage((p) => Math.max(0, p - 1))}
              >
                ← Prev
              </Button>
              <Button
                disabled={loading || events.length < eventsLimit}
            ...
            {codes.map((c) => (
              <tr key={c.id} className="border-b">
                ...
                <td className="p-2">
                  {c.active ? (
                    <Button
                      onClick={() => openReason(c.owner_user_id || '', 'deactivate')}
                      size="sm"
                    >
                      Deactivate
                    </Button>
                  ) : (
                    <Button onClick={() => openReason(c.owner_user_id || '', 'activate')} size="sm">
                      Activate
                    </Button>
                  )}
                </td>
      ...
      {reasonOpen && (
        <Modal onClose={() => setReasonOpen(false)}>
          <div className="p-4 space-y-2">
            ...
            <TextInput value={reasonText} onChange={(e) => setReasonText(e.target.value)} />
            <div className="flex justify-end gap-2 mt-2">
              <Button onClick={() => setReasonOpen(false)}>Cancel</Button>
              <Button onClick={confirmReason} variant="primary">
                Confirm
              </Button>
            </div>
          </div>

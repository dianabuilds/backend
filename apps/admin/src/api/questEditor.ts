...
export async function getVersion(versionId: string): Promise<VersionGraphOutput> {
  const res = await api.get<VersionGraphOutput>(`/admin/quests/versions/${versionId}`);
  return res.data!;
}

export async function putGraph(versionId: string, graph: VersionGraphInput): Promise<void> {
...
export async function validateVersion(versionId: string): Promise<ValidateResult> {
  const res = await api.post<ValidateResult>(`/admin/quests/versions/${versionId}/validate`);
  return res.data!;
}
...
export async function autofixVersion(versionId: string): Promise<AutofixReport> {
  const res = await api.post<AutofixReport>(`/admin/quests/versions/${versionId}/autofix`);
  return res.data!;
}
...
export async function updateQuestMeta(questId: string, patch: QuestUpdate): Promise<QuestOut> {
  const res = await api.patch<QuestOut>(`/admin/quests/${encodeURIComponent(questId)}/meta`, patch);
  return res.data!;
}

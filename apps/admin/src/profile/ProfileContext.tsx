...
export function ProfileProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        // Backend пока отдаёт default_account_id
      ...
      try {
        // Временная точка совместимости: /profiles возвращает тот же набор, что и /accounts
        const res = await api.get<Profile[] | { accounts: Profile[] }>("/profiles");
        const payload = Array.isArray(res.data) ? res.data : res.data?.accounts || [];
        const global = payload.find((it) => (it as any).type === "global" || it.slug === "global");
        if (global) {

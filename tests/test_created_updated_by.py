...
@pytest.mark.asyncio
async def test_created_updated_by_fields(
    db_session: AsyncSession, test_user, admin_user
):
    ...
    await db_session.run_sync(
        lambda s: Achievement.__table__.create(s.bind, checkfirst=True)
    )
    await db_session.run_sync(
        lambda s: WorldTemplate.__table__.create(s.bind, checkfirst=True)
    )
    await db_session.run_sync(
        lambda s: Character.__table__.create(s.bind, checkfirst=True)
    )
    ...
    quest = await create_quest(
        db_session,
        payload=QuestCreate(title="q", nodes=[], custom_transitions={}),
        author=test_user,
        workspace_id=ws.id,
    )
    assert quest.created_by_user_id == test_user.id
    quest = await update_quest(
        db_session, quest_id=quest.id, payload=QuestUpdate(title="q2"), actor=test_user
    )
    assert quest.updated_by_user_id == test_user.id

    # Achievement
    svc = AchievementsAdminService(AchievementsRepository(db_session))
    ach = await svc.create(
        db_session, ws.id, {"code": "a1", "title": "A", "condition": {}}, admin_user.id
    )
    ...
    world = await worlds_svc.create_world(
        db_session, ws.id, {"title": "W"}, admin_user.id
    )
    assert world.created_by_user_id == admin_user.id
    world = await worlds_svc.update_world(
        db_session, ws.id, world.id, {"description": "d"}, admin_user.id
    )
    assert world.updated_by_user_id == admin_user.id
    character = await worlds_svc.create_character(
        db_session, world.id, ws.id, {"name": "C"}, admin_user.id
    )
    assert character.created_by_user_id == admin_user.id
    character = await worlds_svc.update_character(
        db_session, character.id, ws.id, {"role": "hero"}, admin_user.id
    )
    assert character.updated_by_user_id == admin_user.id

    # cleanup tables
    await db_session.run_sync(
        lambda s: Character.__table__.drop(s.bind, checkfirst=True)
    )
    await db_session.run_sync(
        lambda s: WorldTemplate.__table__.drop(s.bind, checkfirst=True)
    )
    await db_session.run_sync(
        lambda s: Achievement.__table__.drop(s.bind, checkfirst=True)
    )

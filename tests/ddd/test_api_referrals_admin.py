...
def test_referrals_admin_codes_and_events(app_client):
    ...
    act = app_client.post(f"/v1/admin/referrals/codes/{uid}/activate", json={"reason": "init"}, headers=headers)
    assert act.status_code == 200, act.text
    de = app_client.post(f"/v1/admin/referrals/codes/{uid}/deactivate", json={"reason": "off"}, headers=headers)

...
## 6. UI редактирования блока

Компонент редактирования блока — основная рабочая поверхность контент-редактора. Цели: быстро понять статус и происхождение блока, изменить метаданные, заполнить локализованные данные и оценить превью. Текущая версия перегружена повторяющейся служебной информацией и не показывает структуру `data`, поэтому любые новые параметры компонента приходится вносить руками через JSON. Требования к новой версии:

- **Единая шапка-сводка.** Первый экран содержит только ключевые бейджи: `title`, `scope`, `section`, статус публикации, текущая версия, тип блока (library/shared/manual). Рядом — компактное сообщение «Показывается на страницах: N» и индикатор готовности локалей. Все второстепенные сведения (ID, slug, ссылка на библиотеку) доступны через tooltip.
- **Карточка источника.** Блок, пришедший из библиотеки, показывает `library_source`: `key`, `section`, `locale`, `updated_at`, автора. Состояния синхронизации: `synced`, `has_updates` (появляется CTA «Применить обновления» + краткий changelog), `detached`. Переход в ручной режим — явный toggle с описанием последствий. Ссылки «Открыть в библиотеке»/«Просмотр» перемещаются сюда.
- **Карточка метаданных.** Поля `title`, `identifier`/`slug`, `scope`, `section`, `is_template`, `status`, `owners`, `tags`. Валидация inline, без дублирования описательного текста. Переключатель «Активировать блок» расположен здесь же и отражает бизнес-состояние (`draft`, `published`, `archived`).
- **Редактор данных по схеме.** Основная часть панели строится автоматически по JSON Schema компонента. Для каждой группы полей используются расклады «вкладки/стек» из схемы. Схема берётся либо напрямую из блоковой библиотеки (`GET /components/{key}/schema`), либо из `meta.component_schema`. При добавлении новых параметров фронт подтягивает обновлённую схему → форма обновляется без доработок.
  - Для локалей используются вкладки `RU/EN/...` с индикаторами заполненности. Обязательные поля подсвечиваются до заполнения; публикация блокируется, если хотя бы одна требуемая локаль неполна.
  - Поля поддерживают контекстные хелперы (placeholder из схемы, `examples`, `description`). Длинный справочный текст скрыт в collapsible «Описание поля».
  - JSON‑вид остаётся доступным в отдельной вкладке «Сырые данные» для power-user'ов, но перестаёт быть единственным способом добавить новые свойства.
- **Превью.** Справа располагается живой предпросмотр: iframe с `/blocks/{id}/preview?locale=<tab>`. Если компонент предоставляет `thumbnail`, вместо пустого блока выводим изображение + кнопку «Обновить превью».
- **История и аудит.** Нижняя часть экрана показывает ленту действий (`updated_at/by`, версии, публикации, ручной режим). Так мы сохраняем полезную информацию, но не раздуваем главный столбец.

Таким образом, форма концентрируется на реальном контенте: редактор сразу видит, какие поля требует компонент, может переключаться между локалями и управлять связью с библиотекой. Любое новое поле, описанное в JSON Schema, автоматически попадает в UI, поэтому проблема «невозможно прокинуть параметр» исчезает. Служебные детали остаются доступными, но не занимают ценное место наверху и внизу блока.

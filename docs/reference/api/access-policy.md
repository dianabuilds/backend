...
| `finance_ops` *(новая)* | Финансовые операции и биллинг | `/v1/billing/admin/*`, отчёты, провайдеры (`./catalog.md:112`) |
...
¹ На текущий момент все `/v1/billing/admin/*` маршруты требуют `admin` (`./catalog.md:112`). Политика подразумевает создание роли `finance_ops` и делегирование к ней.  
...
1. Завести роль `finance_ops`, обновить IAM и биллинг-роуты (`./catalog.md:112`).  

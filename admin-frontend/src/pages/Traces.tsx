  ...
  return (
    <div>
      ...
      <div className="mb-3 grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-2">
        ...
        <input value={type} onChange={(e) => setType(e.target.value)} placeholder="type" className="border rounded px-2 py-1" />
            ...
            {data?.map((t) => (
              <tr key={t.id} className="border-b hover:bg-gray-50 dark:hover:bg-gray-800">
                ...
                <td className="p-2 space-x-2">
                  <button onClick={async () => { await anonymizeTrace(t.id); refresh(); }} className="text-blue-600">Anon</button>
                  <button onClick={async () => { await deleteTrace(t.id); refresh(); }} className="text-red-600">Del</button>
                </td>
              </tr>
            ))}
            {(data?.length || 0) === 0 && (
              <tr>
                <td colSpan={10} className="p-4 text-center text-gray-500">No traces found</td>
              </tr>
            )}
          </tbody>
        </table>

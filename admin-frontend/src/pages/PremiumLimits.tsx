    ...
    <div className="p-4 space-y-4">
      ...
      <div className="rounded border p-3">
        ...
        <div className="flex items-center gap-2">
          <input className="rounded border px-2 py-1 w-[420px]" placeholder="User ID (UUID)" value={userId} onChange={(e) => setUserId(e.target.value)} />
          <button onClick={load} disabled={!userId || loading} className="text-sm px-3 py-1.5 rounded bg-gray-100 hover:bg-gray-200 disabled:opacity-50">
            Проверить
          </button>
        </div>
        ...
        {data ? (
          <div className="mt-3 text-sm">
            ...
            <pre className="mt-2 bg-gray-50 rounded p-2 text-xs whitespace-pre-wrap">{JSON.stringify(data.limits, null, 2)}</pre>
          </div>
        ) : null}
      </div>
    </div>

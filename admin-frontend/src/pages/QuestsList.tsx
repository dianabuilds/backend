...
export default function QuestsList() {
  ...
  const [publishAccess, setPublishAccess] = useState<"premium_only" | "everyone" | "early_access">("everyone");
  ...
  const onPublish = async (id: string) => {
    try {
      setPublishing(true);
      await publishQuest(id, { access: publishAccess, cover_url: publishCover || undefined });
      setPublishing(false);
      setActiveQuestId(null);
      setReport(null);
      await load();
          ...
          <tbody>
            {items.map((q) => (
              <tr key={q.id} className="border-t border-gray-200 dark:border-gray-800">
                ...
                <td className="py-2 pr-4">{typeof q.cost_generation === "number" ? (q.cost_generation / 100).toFixed(2) : "-"}</td>
                <td className="py-2 pr-4">{new Date(q.created_at).toLocaleString()}</td>
                <td className="py-2 pr-4">
                  {q.published_at ? (
                    <span className="inline-flex items-center px-2 py-0.5 rounded bg-green-100 text-green-800">Published {new Date(q.published_at).toLocaleDateString()}</span>
                  ) : (
                    <span className="inline-flex items-center px-2 py-0.5 rounded bg-yellow-100 text-yellow-800">Draft</span>
                  )}
                </td>
                <td className="py-2 pr-4 space-x-2">
                  <button className="px-2 py-1 rounded border" onClick={() => onNewDraft(q.id)}>New draft</button>
                  <button className="px-2 py-1 rounded border" onClick={() => onValidate(q.id)}>Validate</button>
                  {!q.published_at && (
                    <button className="px-2 py-1 rounded border" onClick={() => { setActiveQuestId(q.id); setPublishAccess("everyone"); setPublishCover(""); setReport(null); }}>
                      Publish…
                    </button>
                  )}
                </td>
            ...
            {items.length === 0 && (
              <tr><td className="py-6 text-gray-500" colSpan={5}>No quests yet. Click “New quest” to create one.</td></tr>
            )}
          </tbody>
        </table>
        ...
        {(activeQuestId && (report || !items.find(i => i.id === activeQuestId)?.published_at)) && (
          <div className="mt-4 rounded border p-3">
            <div className="flex items-center justify-between">
              ...
              <button className="text-sm text-gray-600" onClick={() => { setActiveQuestId(null); setReport(null); }}>Close</button>
            </div>

            {report && (
              <div className="mt-2">
                <div className="mb-2">
                  <span className="mr-4">Errors: <b>{report.errors}</b></span>
                  <span>Warnings: <b>{report.warnings}</b></span>
                </div>
                <div className="max-h-64 overflow-auto border rounded">
                  <table className="min-w-full text-sm">
                    <thead className="text-left">
                      <tr><th className="p-2">Level</th><th className="p-2">Code</th><th className="p-2">Message</th></tr>
                    </thead>
                    <tbody>
                      ...
                      {report.items.length === 0 && <tr><td className="p-2 text-gray-500" colSpan={3}>No issues</td></tr>}
                    </tbody>
                  </table>
                </div>
              </div>
            ...
            {!items.find(i => i.id === activeQuestId)?.published_at && (
              <div className="mt-3">
                ...
                <div className="flex flex-wrap items-center gap-2">
                  <select className="border rounded px-2 py-1" value={publishAccess} onChange={(e) => setPublishAccess(e.target.value as any)}>
                  ...
                  <button
                    ...
                    title={((report?.errors ?? 0) > 0) ? "Fix validation errors before publishing" : "Publish quest"}
                  >
                    {publishing ? "Publishing…" : "Publish"}
                  </button>
                  {!report && (
                    <button className="px-3 py-1 rounded border" onClick={() => activeQuestId && onValidate(activeQuestId)}>
                      Run validation
                    </button>
                  )}
                  <button
  ...
  return (
    <PageLayout title="Quests" subtitle="Управление квестами">
      <div className="mb-3 flex flex-wrap items-center gap-2">
        ...
        <select className="border rounded px-2 py-1" value={lenFilter} onChange={(e) => setLenFilter(e.target.value as any)}>
        ...
        <button className="px-3 py-1 rounded border" onClick={onApplyFilters}>Apply</button>
      </div>
      {content}
    </PageLayout>
  );

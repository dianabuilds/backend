...
## Статус на 2025-10-07

- Nodes API распилена по слоям: HTTP-фасад `domains/product/nodes/api/http.py` делегирует в `public/` модули, бизнес-правила вынесены в `application/use_cases/**`, SQL-интеграция — в `infrastructure/*`. Добавлены unit-тесты `tests/unit/nodes/test_use_cases.py`.
- Navigation API приведена к тому же шаблону: `navigation/api/http.py` регистрирует `public`/`admin` пакеты, use-case слой (`transition.py`, `relations_admin.py`) отделён от FastAPI, инфраструктура закрыта в `infrastructure/engine.py` и `relations.py`. Unit-тесты лежат в `tests/unit/navigation`.
- Документация (`apps/backend/ARCHITECTURE.md`) обновлена, отражая новую структуру и распределение ответственности.
- Billing: слой use-case переведён на фасад `BillingUseCases` со вспомогательными портами `BillingSettingsUseCase`, SQL-инфраструктура вынесена в `infrastructure/sql`, настройки потребляют только портовые зависимости. Добавлены unit-/smoke-тесты (`tests/unit/billing`, `tests/smoke/test_api_billing.py`).

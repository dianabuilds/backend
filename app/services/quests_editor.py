...
def validate_graph(nodes: List[GraphNode], edges: List[GraphEdge]) -> ValidateResult:
    errors: List[str] = []
    warnings: List[str] = []
    ...
    # check edges refer existing nodes
    ...
    # dead-ends: nodes without outgoing edges (except for end)
    for n in nodes:
...
def simulate_graph(nodes: List[GraphNode], edges: List[GraphEdge], payload: SimulateIn) -> SimulateResult:
    # Простейшая симуляция: идём по первому допустимому ребру, условия считаем true
    ...
    while cur and cur.key not in visited:
        ...
        if not outs:
            break
        # Выбираем первое ребро (условия считаем истинными на MVP)
        nxt = outs[0]

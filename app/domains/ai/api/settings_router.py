...
compat_router = APIRouter(prefix="/admin/ai/quests", tags=["admin-ai-settings-compat"], responses=ADMIN_AUTH_RESPONSES)

admin_required = require_admin_role()


@router.get("/settings")
async def get_settings(_=Depends(admin_required), db: AsyncSession = Depends(get_db)) -> Dict[str, Any]:
...
async def put_settings(
    ...
    api_key: Optional[str] = payload.get("api_key", None)
...
async def get_settings_compat(_=Depends(admin_required), db: AsyncSession = Depends(get_db)) -> Dict[str, Any]:
    service = SettingsService(AISettingsRepository(db))
    return await service.get_ai_settings()

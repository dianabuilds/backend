...
class TagAdminRepository(ITagAdminRepository):
    ...
    async def add_alias(self, tag_id: UUID, alias_norm: str, type_: str = "synonym") -> TagAlias:
        exists = (await self._db.execute(select(TagAlias).where(TagAlias.alias == alias_norm))).scalars().first()
        if exists:
            return exists
    ...
    async def list_aliases(self, tag_id: UUID) -> list[TagAlias]:
        res = await self._db.execute(select(TagAlias).where(TagAlias.tag_id == tag_id).order_by(TagAlias.alias.asc()))
    ...
    async def dry_run_merge(self, from_id: UUID, to_id: UUID) -> Dict[str, Any]:
        ...
        cnt = (await self._db.execute(select(func.count(NodeTag.node_id)).where(NodeTag.tag_id == from_id))).scalar() or 0
        aliases = (await self._db.execute(select(func.count(TagAlias.id)).where(TagAlias.tag_id == from_id))).scalar() or 0
        return {
    ...
    async def apply_merge(self, from_id: UUID, to_id: UUID, actor_id: str | None, reason: str | None) -> Dict[str, Any]:

...
class InMemoryLLMMetricsSink(ILLMMetricsSink):
    ...
    def prometheus(self) -> str:
        ...
        for (metric, provider, model, stage), cnt in self.counters.items():
            lines.append(f'label="deprecated" llm_calls_total{{type="{metric}",provider="{provider}",model="{model}",stage="{stage}"}} {cnt}')
        ...
        for (provider, model, stage), s in self.latency_sum.items():
            ...
            avg = s / max(c, 1)
            lines.append(f'llm_latency_ms{{provider="{provider}",model="{model}",stage="{stage}"}} {avg}')
        ...
        for (provider, model, stage, t), s in self.tokens_sum.items():
            lines.append(f'llm_tokens_total{{provider="{provider}",model="{model}",stage="{stage}",type="{t}"}} {s}')
        ...
        for (provider, model, stage), s in self.cost_sum.items():
            lines.append(f'llm_cost_usd_total{{provider="{provider}",model="{model}",stage="{stage}"}} {s}')
        return "\n".join(lines) + "\n"
